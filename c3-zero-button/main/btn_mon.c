  ...
  for (;;) {
    ...
    if (xQueueReceive(g_gpio_evt_queue, &hdlr_id, portMAX_DELAY)) {
      if (hdlr_id < g_hdls_sz) {
        ...
        const bool pressed = !gpio_get_level(g_hdlrs[hdlr_id].gpio);
        ESP_LOGD(TAG, "GPIO %d is %s", g_hdlrs[hdlr_id].gpio, pressed ? "pressed" : "released");
        g_hdlrs[hdlr_id].callback(pressed, g_usr_arg);
      } else {
