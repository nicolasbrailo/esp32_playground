...
static void bt_name_read_finish_(uint16_t conn_handle, struct bt_name_read_state *s,
                                        const char *name, size_t len) {
  s->user_cb(conn_handle, name, len, s->user_arg);
  free(s);
}

static int bt_name_read_value_cb_(uint16_t conn_handle, const struct ble_gatt_error *error,
...
static int bt_name_read_svc_cb_(uint16_t conn_handle, const struct ble_gatt_error *error,
  ...
  if (error->status == 0 && svc != NULL && !s->svc_found) {
    ...
    int rc = ble_gattc_disc_chrs_by_uuid(conn_handle, svc->start_handle, svc->end_handle,
                                         &BT_DEVICE_NAME_CHR_UUID.u, bt_name_read_chr_cb_, s);
    if (rc != 0) {
      bt_name_read_finish_(conn_handle, s, NULL, 0);
    }
    return 0;
