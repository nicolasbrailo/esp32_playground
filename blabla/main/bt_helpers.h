...
inline static const char *bt_hci_disconnect_reason_str(int reason) {
  int hci = (reason >= BLE_HS_ERR_HCI_BASE && reason < BLE_HS_ERR_HCI_BASE + 0x100)
...
inline static const char *bt_att_err_str(int status) {
  int att = (status >= BLE_HS_ERR_ATT_BASE && status < BLE_HS_ERR_ATT_BASE + 0x100)
                ? status - BLE_HS_ERR_ATT_BASE
                : status;
  switch (att) {
  case 0x00: return "Success";
